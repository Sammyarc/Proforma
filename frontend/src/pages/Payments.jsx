...
const Payments = () => {
  ...
  const getInvoicePreviewImage = (url) => {
    if (!url || !url.includes("/upload/"))
      return "https://placehold.co/600x400/png";
    const transformedUrl = url
      .replace("/upload/", "/upload/pg_1/format/png/")
      .replace(".pdf", ".png");
                ...
                {invoices.map((invoice) => (
                  <tr key={invoice._id} className="border-b border-gray-800">
                    <td className="py-4 px-3">
                      <div className="relative w-24 h-20 rounded-md">
                        <img
                          src={getInvoicePreviewImage(invoice.invoiceUrl)}
